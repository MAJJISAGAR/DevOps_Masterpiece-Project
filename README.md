...
```bash
docker build -t sagar520/spring-app1:${VERSION}-${GIT_COMMIT} .
```

The image is pushed to DockerHub using:

```bash
docker push sagar520/spring-app1:${VERSION}-${GIT_COMMIT}
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: spring-app1
        image: sagar520/spring-app1:${VERSION}-${GIT_COMMIT}
        ports:
        - containerPort: 8080
```
